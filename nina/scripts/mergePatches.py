...
source_path = "/tmp/udata/nina/presets/patches/"
files = []
direcs = []
...
# get layer files
path = "/udata/nina/presets/layers/"
source_path = "/tmp/udata/nina/presets/layers/"
try:
    source_dirs = os.listdir(source_path)
except:
    print("no layers to extract")
    source_dirs = []
dst_dirs = os.listdir(path)

for file in source_dirs:
    file_trim = file[0:3]
    os.system("rm " + path + file_trim + "*")
    print("remove " + path + file_trim)
    os.system("cp " + source_path + file_trim + "* " + path)
    print("cp " + source_path + file_trim + "* " + path)
print("")
