...
NINA_WT_LENGTH = 2048
NINA_MAX_WAVES = 256
NINA_MAX_SAMPLES = NINA_WT_LENGTH * NINA_MAX_WAVES
...
def check_wave(path):
    try:
        ...
        print(wt_length)
        #handle SERUM format
        if((wt_length == NINA_WT_LENGTH or  not(file_is_wave_edit_length))):
            print("load serum wt")
            samplerate, wave_table = wavfile.read(path)
            shape = np.shape(wave_table)
            print(path)
            ...
            if(len(shape) > 1):
                ...
                return False
            length = shape[0]
            if((length % NINA_WT_LENGTH) != 0 ):
                print("wrong length")
                return False
            if((length / NINA_WT_LENGTH) > NINA_MAX_WAVES):
                print("wt too long")
                return False
            return True
        ...
        #handle WAVEEDIT format
            ...
            data = sps.resample_poly(wave_table,NINA_WT_LENGTH/WAVEEDIT_WT_LENGTH,1)
